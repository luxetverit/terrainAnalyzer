...
def run_full_analysis(user_gdf_original, selected_types, subbasin_name):
    ...
    if any(item in selected_types for item in ['soil', 'hsg', 'landcover']):
        ...
        if 'soil' in selected_types:
            sql_soil = f"SELECT ST_Intersection(t1.geometry, ST_GeomFromText('{user_wkt}', 5186)) AS intersected_geom, t1.* FROM public.kr_soil_map AS t1 WHERE ST_Intersects(t1.geometry, ST_GeomFromText('{user_wkt}', 5186));"
            gdf = gpd.read_postgis(sql_soil, engine, geom_col='intersected_geom')
            gdf = gdf.drop(columns=['geometry'], errors='ignore').rename(columns={'intersected_geom': 'geometry'}).set_geometry('geometry')
            dem_results['soil'] = {'gdf': gdf}

        if 'hsg' in selected_types:
            sql_hsg = f"SELECT ST_Intersection(t1.geometry, ST_GeomFromText('{user_wkt}', 5186)) AS intersected_geom, t1.* FROM public.kr_hsg_map AS t1 WHERE ST_Intersects(t1.geometry, ST_GeomFromText('{user_wkt}', 5186));"
            gdf = gpd.read_postgis(sql_hsg, engine, geom_col='intersected_geom')
            gdf = gdf.drop(columns=['geometry'], errors='ignore').rename(columns={'intersected_geom': 'geometry'}).set_geometry('geometry')
            dem_results['hsg'] = {'gdf': gdf}

        if 'landcover' in selected_types:
            sql_landcover = f"SELECT ST_Intersection(t1.geometry, ST_GeomFromText('{user_wkt}', 5186)) AS intersected_geom, t1.* FROM public.kr_landcover_map AS t1 WHERE ST_Intersects(t1.geometry, ST_GeomFromText('{user_wkt}', 5186));"
            gdf = gpd.read_postgis(sql_landcover, engine, geom_col='intersected_geom')
            gdf = gdf.drop(columns=['geometry'], errors='ignore').rename(columns={'intersected_geom': 'geometry'}).set_geometry('geometry')
            dem_results['landcover'] = {'gdf': gdf}
