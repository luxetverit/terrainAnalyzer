...
from utils.plot_helpers import (generate_custom_intervals,
                                ...
                                generate_aspect_bins) # Note: aspect bins only
...
def extract_points_from_geometries(gdf, elevation_col='elevation'):
    ...
    for _, row in gdf.iterrows():
        ...
        if geom is None or geom.is_empty or z is None or not np.isfinite(z):
            continue
        geoms_to_process = [geom] if not geom.geom_type.startswith('Multi') else list(geom.geoms)
...
def get_pixel_size_from_db(engine, subbasin_name, default_size=1.0):
    ...
    try:
        ...
        with engine.connect() as connection:
            query = text("SELECT pixel_size_m FROM subbasin_pixel_size WHERE subbasin = :subbasin")
            result = connection.execute(query, {"subbasin": subbasin_name}).scalar_one_or_none()
...
def run_full_analysis(user_gdf_original, selected_types, subbasin_name):
    ...
    expanded_bounds = (user_bounds[0] - buffer_m, user_bounds[1] - buffer_m, user_bounds[2] + buffer_m, user_bounds[3] + buffer_m)
            ...
            if analysis_type in selected_types or (analysis_type == 'elevation' and dem_needed):
                with rasterio.open(temp_files_to_clean[analysis_type]) as src:
                    ...
                    if analysis_type == 'elevation' or analysis_type == 'slope':
                        ...
                        num_divisions = 10 if grid_stats.get('area', 0) > area_threshold else 8
                        if analysis_type == 'elevation':
                            palette_name = f'elevation_{num_divisions}'
                            bins, labels = generate_custom_intervals(grid_stats['min'], grid_stats['max'], num_divisions)
                        else: # slope
                            palette_name = f'slope_{num_divisions}'
                            bins, labels = generate_slope_intervals(num_divisions)
    ...
    if any(item in selected_types for item in ['soil', 'hsg', 'landcover']):
        ...
        if 'soil' in selected_types:
            ...
            dem_results['soil'] = {'gdf': gpd.read_postgis(sql_soil, engine, geom_col='geom')}
        if 'hsg' in selected_types:
            ...
            dem_results['hsg'] = {'gdf': gpd.read_postgis(sql_hsg, engine, geom_col='geom')}
        if 'landcover' in selected_types:
            ...
            dem_results['landcover'] = {'gdf': gpd.read_postgis(sql_landcover, engine, geom_col='geom')}
    return dem_results