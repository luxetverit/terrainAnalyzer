    ...
    try:
        ...
        if file_ext == ".dxf":
            ...
            try:
                ...
                shp_files = [os.path.join(root, file) for root, _, files in os.walk(extract_dir) for file in files if file.lower().endswith(".shp")]
...
def process_shp_directory(dir_path, epsg_code):
    ...
    """
    try:
        shp_files = [os.path.join(root, file) for root, _, files in os.walk(dir_path) for file in files if file.lower().endswith(".shp")]
        if not shp_files:
