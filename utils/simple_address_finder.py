...
def get_location_name(gdf, original_epsg, api_key):
    ...
    try:
        ...
        if response.status_code != 200:
            return {
                "road_address": f"주소 조회 실패 (HTTP 상태 코드: {response.status_code})",
                "address": f"오류 메시지: {response.text}",
                "region": "대한민국",
        ...
        if not data or "documents" not in data or len(data["documents"]) == 0:
            return {
                "road_address": "해당 좌표에 대한 주소 정보가 없습니다.",
                "address": "(산, 바다 등 주소가 없는 지역일 수 있습니다)",
                "region": "대한민국",
                "original_x": original_x,
                "original_y": original_y,
                "lon": lon,
        ...
        road_address = (
            road_address_data.get("address_name", "주소 정보를 찾을 수 없습니다")
            if road_address_data
            else "주소 정보를 찾을 수 없습니다"
        )
