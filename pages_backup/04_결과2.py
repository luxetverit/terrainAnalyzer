...
if "analysis_results" not in st.session_state:
    st.session_state.analysis_results = {
        "done": True,
        "message": "분석이 완료되었습니다!",
        "palettes": {"elevation": "spectral", "slope": "terrain"},  # 기본값  # 기본값
    }
...
with col1:
    if "elevation" in selected_types:
        ...
        st.markdown(
            f"<div class='palette-label'>{ALL_PALETTES[elevation_palette]['name']}</div>",
            unsafe_allow_html=True,
        )
...
with col2:
    if "slope" in selected_types:
        ...
        st.markdown(
            f"<div class='palette-label'>{ALL_PALETTES[slope_palette]['name']}</div>",
            unsafe_allow_html=True,
        )
...
if selected_types:
    tabs = st.tabs(
        [
            (
                ALL_PALETTES[elevation_palette]["name"]
                if type_key == "elevation"
                else (
                    ALL_PALETTES[slope_palette]["name"]
                    if type_key == "slope"
                    else type_key
                )
            )
    ...
    for i, type_key in enumerate(selected_types):
        with tabs[i]:
            ...
            if type_key == "elevation":
                ...
                bounds = (0, 0, 100, 100)
                # 표고 시각화 생성
                fig = create_elevation_heatmap(
                    data_array, bounds, stats, elevation_palette
                )
                st.pyplot(fig)
