...
if 'dem_results' not in st.session_state:
    ...
    with st.status("지형 분석 진행 중...", expanded=True) as status:
        try:
            ...
            # 1b. Fetch contour data from DB using the user's boundary directly
            user_bounds = user_gdf_reprojected.total_bounds
            bbox_wkt = (
                f"POLYGON(({user_bounds[0]} {user_bounds[1]}, "
                f"{user_bounds[2]} {user_bounds[1]}, "
                f"{user_bounds[2]} {user_bounds[3]}, "
                f"{user_bounds[0]} {user_bounds[3]}, "
                f"{user_bounds[0]} {user_bounds[1]}))"
            )
            ...
            if dem_needed:
                ...
                status.write("3/5: DEM 보간 작업 중...")
                minx, miny, maxx, maxy = user_gdf_reprojected.total_bounds
                pixel_size = 1.0
