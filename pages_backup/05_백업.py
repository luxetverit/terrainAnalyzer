...
if not valid_selected_types:
    ...
    for i, analysis_type in enumerate(valid_selected_types):
        with tabs[i]:
            ...
            st.markdown("#### 분석도")
            if grid is not None:
                # 각 분석 유형에 대한 컬러맵 옵션 정의
                cmap_options = {
                    'elevation': ['terrain', 'gist_earth', 'viridis', 'Spectral_r'],
                    'slope': ['viridis', 'inferno', 'magma', 'RdYlGn_r'],
                    'aspect': ['hsv', 'twilight_shifted', 'circular']
                }

                # 현재 분석 유형에 맞는 컬러맵 목록 가져오기
                    ...
                    with cmap_tabs[j]:
                        ...
                        if analysis_type == 'elevation':
                            if stats and stats.get('min') is not None and stats.get('max') is not None:
                                levels = np.linspace(
                                    stats.get('min'), stats.get('max'), 15)
                                contour = ax.contour(
                                    grid, levels=levels, colors='k', alpha=0.4, linewidths=0.7)
                        ...
                        add_north_arrow(ax)
                        add_scalebar(
                            ax, dx=st.session_state.get('pixel_size', 1.0))
                        ax.axis('off')
                        st.pyplot(fig)
